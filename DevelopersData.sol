...
contract DeveloperData{
    mapping(address => uint[]) totalNNs;
    mapping(address => mapping(uint => string)) totalNNsNames;
    mapping(uint => uint) public rating;
    
    function setRating(uint _id, uint _rating){
        if( _rating > 0 && _rating <= 5 && !(hashCompareWithLengthCheck(totalNNsNames[msg.sender][_id],""))){
            rating[_id] = _rating;
        } 
        
    } 
    function getRating(uint _id) returns (uint){
        if (!(hashCompareWithLengthCheck(totalNNsNames[msg.sender][_id],""))){
